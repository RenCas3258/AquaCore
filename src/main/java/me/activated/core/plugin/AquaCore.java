    ...
    public void onEnable() {
        ...
        try {
            ...
            Bukkit.getConsoleSender().sendMessage(ChatColor.translateAlternateColorCodes('&',
                    "&cLicense for &a" + this.getDescription().getName() +
                            " v" + this.getDescription().getVersion() + " &ccouldn't be passed. " +
                            "&cAre you using latest version of it? &eContact plugin developer if you think this is an issue."));
            Bukkit.getPluginManager().disablePlugin(this);
            return;
        }
        if (!this.registerManager.getPlayerListener().iOw().equalsIgnoreCase("ao1plambju28an")) {
            return;
        }
