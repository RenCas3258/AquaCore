package me.activated.core.menu.menu;
...
public abstract class AquaMenu {
    ...
    public void update(Player player) {
        ...
        if (currentlyOpenedMenu != null && Color.translate(currentlyOpenedMenu.getName(player))
                .equals(current.getTitle()) && current.getSize() == this.getInventorySize(this.slots)) {
            inventory = current;
            passed = true;
        }
        ...
        if (passed) {
            ...
            player.openInventory(inventory);
            Bukkit.getConsoleSender().sendMessage(Color.translate("&cOpened new inventory"));
        }
